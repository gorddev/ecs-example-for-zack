...
export default class Filters {
    ...
    private readonly permanentFilters : Array<PixiFilter>;
...
// *************** Member Functions **************** //
    ...
    public update(mSprite : MapSprite, dt : number) : void {
        ...
        this.dynamicFilters =
            this.dynamicFilters.filter(f => {
                if (f[1](f[0], dt)) {   // First call the dynamic filter's update function.
                    f[0].destroy(); // if true, destroy the filter and remove it from the dynamic filter list.
                    return false;
                } return true;
        });
    ...
    /** Checks to see if there's any more filters left */
    public isDone() : boolean {
        return this.permanentFilters.length === 0 && this.dynamicFilters.length === 0;
    }

    // Called when the Filter object is destructed
    [Symbol.dispose]() {
